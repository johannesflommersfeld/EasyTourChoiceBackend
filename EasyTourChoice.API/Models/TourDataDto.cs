...
public class TourDataDto(
    ...
    float? approachDuration, string? shortDescription, int? difficulty, int? risk, AreaDto? area, float? travelTime,
    int? weatherPreview)
{
    ...
    // calculated fields
    public float? TravelTime {get; set; } = travelTime; // estimated travel time in hours
    public int? WeatherPreview {get; set; } = weatherPreview;
}